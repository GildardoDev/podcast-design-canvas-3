...
// Exits non-zero if any test file fails. No frameworks, no dependencies.
...
let files = [];
try {
  files = readdirSync(testsDir).filter((f) => f.endsWith(".test.js")).sort();
} catch {
...
for (const f of files) {
  const res = spawnSync(process.execPath, [path.join(testsDir, f)], { stdio: "inherit" });
  if (res.status !== 0) {
