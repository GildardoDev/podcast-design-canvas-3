...
// Drives the shipped app in headless Chrome and proves the active #73 workflow:
// upload two speaker videos that carry audio, click the audio mixing controls
// (leveling/clarity/noise) to enable the engine, confirm the live level meter
// shows real signal, change a control and confirm an observable level change
// without re-uploading, then export and decode the exported file to confirm it
// has real video dimensions AND a non-silent audio track that reflects the
// settings. No fixtures or verifier-only paths: media is generated in-browser
...
(async () => {
  ...
  async function makeVideo(name, color, freq) {
    ...
    rec.start();
    for (let i = 0; i < 30; i++) { ctx.fillStyle = color; ctx.fillRect(0,0,320,180); await sleep(45); }
  ...
  const peakLevel = async () => { let m = 0; for (let i = 0; i < 12; i++) { m = Math.max(m, PDC.audio.level()); await sleep(40); } return m; };

  await waitFor(() => window.PDC && window.PDC.audio && document.querySelector('[data-audio-set="leveling:auto"]') && document.querySelector("#export"), "shipped audio controls should exist");

  // Two speaker videos that carry audio.
  uploadTo(document.querySelector('[data-file-bucket="host"]'), await makeVideo("host.webm", "#b91c1c", 330));
  await sleep(100);
  uploadTo(document.querySelector('[data-file-bucket="guest1"]'), await makeVideo("guest.webm", "#047857", 550));
  await sleep(1500);
...
main().catch((e) => { console.error(`verify-audio: ${e.message}`); process.exit(1); });
